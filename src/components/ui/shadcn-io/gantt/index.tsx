...
import { useGanttDragging, useGanttScrollX } from "./hooks";
...
export const GanttColumn: FC<GanttColumnProps> = ({
  ...
  useEffect(() => {
    const updateTop = () => {
      ...
      const newTop = mousePosition.y - (rect?.y ?? 0) - (windowScroll.y ?? 0);
      setTop(newTop);
    };

    const throttledUpdate = throttle(updateTop, 10);
    throttledUpdate();
  ...
  }, [mousePosition.y, windowScroll.y]);
...
export const GanttCreateMarkerTrigger: FC<GanttCreateMarkerTriggerProps> = ({
  ...
  useEffect(() => {
    const updateX = () => {
      const rect = mouseRef.current?.getBoundingClientRect();
      const newX = mousePosition.x - (rect?.x ?? 0) - (windowScroll.x ?? 0);
      setX(newX);
    };

    const throttledUpdate = throttle(updateX, 10);
    throttledUpdate();
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mousePosition.x, windowScroll.x]);
